...
impl LoopService {
    pub fn new(projects_root: PathBuf) -> Self {
        let persistence_path = projects_root
            .join(".native-shell")
            .join("loop-session.state");
        Self {
            projects_root,
            persistence_path,
            session: None,
    ...
    pub fn stop_session(&mut self) -> io::Result<Option<LoopUpdate>> {
        ...
        self.persist_session(&session)?;
        Ok(Some(Self::to_update(
            session,
            vec![String::from("Loop stopped from native shell.")],
        )))
    }
    ...
    pub fn load_persisted_session(&mut self) -> io::Result<Option<LoopUpdate>> {
        ...
        let Some(session) = Self::parse_session(&raw_state) else {
            return Ok(None);
        };
        self.session = Some(session.clone());
    ...
    fn persist_session(&self, session: &LoopSession) -> io::Result<()> {
        let state_dir = self
            .persistence_path
            .parent()
            .ok_or_else(|| io::Error::other("missing persistence parent"))?;
...
mod tests {
    ...
    fn start_and_stop_report_native_loop_controls() {
        ...
        let started = service
            .start_session("project-native", "Native shell project")
            .expect("start");
        assert!(started.running);
        assert!(started
            .events
            .iter()
            .any(|event| event.contains("Iteration 1 completed")));
        assert!(started
            .events
            .iter()
            .any(|event| event.contains("Story blocked")));
        assert!(started
            .events
            .iter()
            .any(|event| event.contains("Rate limit")));
    ...
    fn persisted_session_is_restored_on_startup() {
        ...
        let started = writer
            .start_session("project-restore", "Recoverable project")
            .expect("start");
    ...
    fn stopped_session_is_restored_as_resumable() {
        ...
        let mut writer = LoopService::new(projects_root.clone());
        let started = writer
            .start_session("project-resume", "Resumable project")
            .expect("start");
