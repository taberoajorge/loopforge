...
impl ProjectsService {
    pub fn new(root: Option<PathBuf>) -> Self {
        Self { root: root.unwrap_or_else(Self::default_root) }
    }
    ...
    pub fn create_project(&self, request: CreateProjectRequest) -> io::Result<ProjectRecord> {
        ...
        BackendAdapter::create_project(request, |request| {
            projects::create_project(
                request,
                || format!("proj-{}-{}", Self::timestamp_nanos(), Self::slug(&project_name)),
                Self::timestamp,
    ...
    pub fn project_detail(&self, project_id: &str) -> io::Result<SharedProjectDetail> {
        BackendAdapter::project_detail(project_id, |resolved_id| {
            ...
            let passed_count = stories.iter().filter(|story| story.status == "passed").count();
            let blocked_count = stories.iter().filter(|story| story.status == "blocked").count();
    ...
    fn default_root() -> PathBuf {
        PathBuf::from(env::var("HOME").unwrap_or_else(|_| ".".to_owned())).join(".config/loopforge/projects")
    }
    ...
    fn persist_record(&self, record: CreateProjectRecord) -> io::Result<ProjectRecord> {
        ...
        fs::create_dir_all(&project_dir)?;
        fs::write(project_dir.join("project.txt"), Self::encode_project(&project))?;
        fs::write(project_dir.join("draft.json"), format!("id={}\nname={}\nstep={}\ndescription={}\n", project.id, project.name, project.wizard_step.clone().unwrap_or_else(|| "describe".to_owned()), project.description))?;
        Ok(project)
    }
        ...
        for line in content.lines() {
            if let Some((key, value)) = line.split_once('=') {
                match key.trim() {
                    ...
                    "wizard_step" => project.wizard_step = (!value.trim().is_empty()).then(|| value.trim().to_owned()),
                    _ => {}
                }
            }
        }
    ...
    fn update_status(&self, project_id: &str, status: &str) -> io::Result<()> {
        ...
        fs::write(self.root.join(project_id).join("project.txt"), Self::encode_project(&project))
    }
    ...
    fn find_project(grouped: &SharedProjects, project_id: &str) -> io::Result<ProjectRecord> {
        grouped.active.iter().chain(grouped.paused.iter()).chain(grouped.completed.iter()).chain(grouped.draft.iter()).chain(grouped.archived.iter()).chain(grouped.blocked.iter()).chain(grouped.failed.iter()).find(|project| project.id == project_id).cloned().ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "project not found"))
    }
    fn read_stories(&self) -> Vec<IterationStory> { Vec::new() }
    fn timestamp() -> String { Self::timestamp_nanos().to_string() }
    fn timestamp_nanos() -> u128 {
        SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos()
    }
    fn slug(value: &str) -> String {
        let compact = value.trim().to_lowercase().chars().map(|character| if character.is_ascii_alphanumeric() { character } else { '-' }).collect::<String>().trim_matches('-').replace("--", "-");
        if compact.is_empty() { String::from("project") } else { compact }
    }
}
...
mod tests {
    ...
    fn temp_root() -> std::path::PathBuf { std::env::temp_dir().join(format!("native-shell-projects-{}", super::ProjectsService::timestamp_nanos())) }
    #[test]
    fn projects_service_uses_shared_project_contracts() {
        let root = temp_root();
        let service = ProjectsService::new(Some(root.clone()));
        let project = service.create_project(CreateProjectRequest { name: String::from("LoopForge"), description: String::from("Backend-owned wizard"), working_directory: String::from("/tmp/loopforge"), wizard_step: Some(String::from("describe")) }).expect("create");
