...
impl NativeShellApp {
    ...
    pub fn start_loop_session(&mut self, project_id: &str) -> io::Result<()> {
        let project_name = self.home.active_projects.iter().find(|project| project.id == project_id).map(|project| project.name.clone()).unwrap_or_else(|| project_id.to_owned());
        self.monitor_state = MonitorState::start(project_id.to_owned(), project_name);
        match self.loop_service.start_session(project_id) {
            Ok(update) => {
                self.monitor_state = self.monitor_state.clone().apply_update(update.session_id, update.running, update.events, update.completed_iterations, update.blocked_states, update.rate_limit_events);
    ...
    pub fn stop_loop_session(&mut self) -> io::Result<()> {
        ...
        self.monitor_state = match update {
            Some(update) => self.monitor_state.clone().apply_update(update.session_id, update.running, update.events, update.completed_iterations, update.blocked_states, update.rate_limit_events),
            None => self.monitor_state.clone().stop(),
        };
        Ok(())
    }
