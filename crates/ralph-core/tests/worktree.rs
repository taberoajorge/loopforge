...
fn init_repo() -> (tempfile::TempDir, PathBuf, String, String) {
    ...
    git(&repo_dir, &["config", "user.email", "loopforge@example.com"]);
    let tracked_contents = "tracked root contents\n".to_string();
    let untracked_contents = "untracked sentinel\n".to_string();
    std::fs::write(repo_dir.join("README.md"), &tracked_contents).expect("tracked file should write");
    git(&repo_dir, &["add", "README.md"]);
    git(&repo_dir, &["commit", "-m", "init"]);
    std::fs::write(repo_dir.join("notes.txt"), &untracked_contents).expect("untracked file should write");
    let canonical_repo_dir = repo_dir.canonicalize().expect("repo dir should canonicalize");
    (temp_dir, canonical_repo_dir, tracked_contents, untracked_contents)
}
...
async fn worktree_setup_and_teardown_are_reentrant() {
    ...
    assert!(worktree_dir.exists(), "worktree directory must exist after setup");
    assert_eq!(
        std::fs::read_to_string(worktree_dir.join("README.md")).unwrap(),
        tracked_contents
    );
    assert_eq!(git(&repo_dir, &["rev-parse", "HEAD"]), repo_head);
    assert_eq!(std::fs::read_to_string(repo_dir.join("README.md")).unwrap(), tracked_contents);
    assert_eq!(std::fs::read_to_string(repo_dir.join("notes.txt")).unwrap(), untracked_contents);
    let first_list = worktree::list(&repo_dir).await.expect("worktree list should load");
    assert_eq!(
        first_list
            .iter()
    ...
    assert!(branch_exists(&repo_dir, worktree_branch), "branch should remain after partial teardown");
    let first_teardown = worktree::list(&repo_dir)
        .await
        .expect("list should succeed after first teardown");
    assert!(first_teardown.iter().all(|entry| entry.path != expected_path));
    ...
    let second_list = worktree::list(&repo_dir).await.expect("second list should load");
    assert_eq!(
        second_list
            .iter()
            .filter(|entry| entry.path == expected_path)
    ...
    assert!(!worktree_dir.exists(), "worktree directory must stay removed");
    assert!(
        !branch_exists(&repo_dir, worktree_branch),
        "branch should be deleted during final teardown"
    );
    let final_list = worktree::list(&repo_dir).await.expect("final list should load");
