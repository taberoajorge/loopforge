...
use crate::plan_engine::args::{agent_env_vars, build_null_stdin_command, build_plan_args, needs_null_stdin};
...
fn record_output(stream: &'static str, bytes: &[u8], tracer: &Option<SessionTracer>, classifier: &Arc<Mutex<ActivityClassifier>>, event_buffer: &Arc<Mutex<Vec<PlanActivityPayload>>>, project_id: &str, last_activity: &Arc<Mutex<Instant>>) {
    let text = String::from_utf8_lossy(bytes);
    if let Some(ref tracer) = tracer {
        let lines: Vec<&str> = text.lines().collect();
        tracer.log(TraceEvent::Output { stream, byte_len: bytes.len(), line_count: lines.len(), first_line_preview: lines.first().map(|line| line.chars().take(120).collect()).unwrap_or_default() });
    }
    output::buffer_text_segments(&text, classifier, event_buffer, project_id, last_activity);
}

pub async fn start_plan<R: Runtime>(app: AppHandle<R>, state: State<'_, PlanSessionsState>, args: StartPlanArgs) -> Result<(), PlanEngineError> {
    if !args.project_dir.exists() { return Err(PlanEngineError::Path(format!("Working directory not found: {}", args.project_dir.display()))); }
    if !args.project_dir.is_dir() { return Err(PlanEngineError::Path(format!("Working directory is not a directory: {}", args.project_dir.display()))); }
    {
        let sessions = state.0.lock().map_err(|_| PlanEngineError::LockPoisoned)?;
        if sessions.sessions.contains_key(&args.project_id) { return Err(PlanEngineError::AlreadyRunning(args.project_id)); }
    }
    ...
    let agent_args = build_plan_args(&args.agent, &plan_prompt, args.project_dir.as_path(), args.model.as_deref(), args.effort.as_deref());
    ...
    let (mut event_rx, child) = if use_null_stdin {
        ...
        app.shell().command("/bin/zsh").args(["-lc", &wrapped]).envs(env_vars).current_dir(&args.project_dir).spawn().map_err(|err| PlanEngineError::Shell(err.to_string()))?
    } else {
        app.shell().command(&agent_binary).args(agent_args).envs(env_vars).current_dir(&args.project_dir).spawn().map_err(|err| PlanEngineError::Shell(err.to_string()))?
    };
    ...
    let plan_abort = monitor::spawn_plan_flush_task(Arc::clone(&classifier), plan_path.clone()).abort_handle();
    let batch_abort = monitor::spawn_batch_flush_task(Arc::clone(&event_buffer), app.clone(), args.project_id.clone()).abort_handle();
    let heartbeat_abort = monitor::spawn_heartbeat_task(app.clone(), Arc::clone(&last_activity), Arc::clone(&state.0), args.project_id.clone(), args.agent.clone(), now, tracer.clone()).abort_handle();
    app_core_plan::register_cleanup(&args.project_id, cleanup_hook(app.clone(), args.project_id.clone(), Arc::clone(&event_buffer), Arc::clone(&classifier), plan_path, tracer.clone(), plan_abort, batch_abort, heartbeat_abort));

    state.0.lock().map_err(|_| PlanEngineError::LockPoisoned)?.sessions.insert(
        args.project_id.clone(),
        PlanSessionEntry { handle: PlanSessionHandle::Shell(child), status: PlanSessionStatus::Running, agent_name: args.agent.clone(), started_at: now, last_activity_at: Arc::clone(&last_activity) },
    );
    ...
    tokio::spawn(async move {
        while let Some(event) = event_rx.recv().await {
            match event {
                CommandEvent::Stdout(ref bytes) => record_output("stdout", bytes, &tracer, &classifier, &event_buffer, &project_id, &last_activity),
                CommandEvent::Stderr(ref bytes) => record_output("stderr", bytes, &tracer, &classifier, &event_buffer, &project_id, &last_activity),
                CommandEvent::Terminated(payload) => {
                    let _: Result<bool, PlanEngineError> = app_core_plan::cleanup_session(
                        &project_id,
                        PlanCleanupReason::ProcessExit { exit_code: payload.code.unwrap_or(1) },
                        |project_id| {
