...
pub fn attach_app(builder: tauri::Builder<tauri::Wry>) -> tauri::Builder<tauri::Wry> {
    services::attach_runtime_aliases(builder, {
        ...
        move |invoke: tauri::ipc::Invoke<tauri::Wry>| match invoke.message.command() {
            ...
            "list_projects" => commands::projects_lifecycle::__cmd__list_projects!(project_commands::list_projects, invoke),
            "get_project_detail" => commands::projects_lifecycle::__cmd__get_project_detail!(project_commands::get_project_detail, invoke),
            _ => fallback(invoke),
        }
    })
}
...
mod project_commands {
    ...
    pub async fn create_project(app: AppHandle, db: State<'_, DbState>, name: String, description: String, working_directory: String, wizard_step: Option<String>) -> Result<Project, ProjectError> {
        let request = app_core_projects::CreateProjectRequest { name: required(name, "name").map_err(ProjectError::Path)?, description: required(description, "description").map_err(ProjectError::Path)?, working_directory: required(working_directory, "working_directory").map_err(ProjectError::Path)?, wizard_step: optional(wizard_step) };
        app_core_projects::create_project(request, || Uuid::new_v4().to_string(), || chrono::Utc::now().to_rfc3339(), |project_id, project_name| { let dir = artifact_dir(&app, project_id)?; init_artifacts(&dir, project_name) }, |record| { let project = Project { id: record.id.clone(), name: record.name.clone(), description: record.description.clone(), status: record.status.clone(), working_directory: record.working_directory.clone(), created_at: record.created_at.clone(), updated_at: record.updated_at.clone(), wizard_step: record.wizard_step.clone() }; let conn = db.0.lock().map_err(|_| ProjectError::Db("Lock poisoned".to_string()))?; conn.execute("INSERT INTO projects (id, name, description, status, working_directory, created_at, updated_at, wizard_step) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)", rusqlite::params![record.id, record.name, record.description, record.status, record.working_directory, record.created_at, record.updated_at, record.wizard_step])?; Ok(project) })
    }

    pub async fn finalize_draft(app: AppHandle, db: State<'_, DbState>, project_id: String) -> Result<(), ProjectError> {
        let project_id = required(project_id, "project_id").map_err(ProjectError::Path)?;
        app_core_projects::finalize_draft(project_id, || chrono::Utc::now().to_rfc3339(), |project_id, updated_at| { let conn = db.0.lock().map_err(|_| ProjectError::Db("Lock poisoned".to_string()))?; conn.execute("UPDATE projects SET wizard_step = NULL, wizard_state_json = NULL, updated_at = ?1 WHERE id = ?2", rusqlite::params![updated_at, project_id])?; Ok(()) }, |project_id| { let draft_path = artifact_dir(&app, project_id)?.join("draft.json"); if draft_path.exists() { let _ = std::fs::remove_file(draft_path); } Ok(()) })
    }

    pub async fn discard_draft(app: AppHandle, db: State<'_, DbState>, project_id: String) -> Result<(), ProjectError> {
        let project_id = required(project_id, "project_id").map_err(ProjectError::Path)?;
        app_core_projects::discard_draft(project_id, |project_id| { let conn = db.0.lock().map_err(|_| ProjectError::Db("Lock poisoned".to_string()))?; Ok(conn.execute("DELETE FROM projects WHERE id = ?1 AND status = 'draft'", rusqlite::params![project_id])? != 0) }, |project_id| { let dir = artifact_dir(&app, project_id)?; if dir.exists() { let _ = std::fs::remove_dir_all(&dir); } Ok(()) }, ProjectError::NotFound)
    }

    pub async fn list_projects(db: State<'_, DbState>) -> Result<ProjectsByStatus, ProjectError> {
        app_core_projects::list_projects(|| { let conn = db.0.lock().map_err(|_| ProjectError::Db("Lock poisoned".to_string()))?; let query = format!("SELECT {PROJECT_COLUMNS} FROM projects ORDER BY updated_at DESC"); let mut stmt = conn.prepare(&query)?; let projects: Vec<Project> = stmt.query_map([], row_to_project)?.filter_map(|result| result.ok()).collect(); Ok(group_projects_by_status(projects)) })
    }

    pub async fn get_project_detail(app: AppHandle, db: State<'_, DbState>, project_id: String) -> Result<ProjectDetail, ProjectError> {
        let project_id = required(project_id, "project_id").map_err(ProjectError::Path)?;
        let detail = app_core_projects::get_project_detail(project_id, |project_id| { let conn = db.0.lock().map_err(|_| ProjectError::Db("Lock poisoned".to_string()))?; let query = format!("SELECT {PROJECT_COLUMNS} FROM projects WHERE id = ?1"); let mut stmt = conn.prepare(&query)?; stmt.query_row(rusqlite::params![project_id], row_to_project).map_err(|_| ProjectError::NotFound(project_id.to_string())) }, |project_id| { let prd_path = artifact_dir(&app, project_id)?.join("prd.json"); Ok(if prd_path.exists() { Prd::load(&prd_path).map(|prd| prd.stories).unwrap_or_default() } else { Vec::new() }) })?;
        Ok(ProjectDetail { project: detail.project, total_stories: detail.total_stories, passed_count: detail.passed_count, blocked_count: detail.blocked_count, pending_count: detail.pending_count, stories: detail.stories })
    }

    fn required(value: String, name: &str) -> Result<String, String> {
        let trimmed = value.trim().to_string();
        if trimmed.is_empty() { return Err(format!("{name} is required")); }
        Ok(trimmed)
    }
