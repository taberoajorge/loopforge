...
export function DescribeForm(props: DescribeFormProps) {
  ...
  return (
    <form
      aria-label="Project description form"
      data-testid="describe-form"
    ...
    >
      <div className="mb-8">
        <h2 className="font-bold font-mono text-lg text-text uppercase tracking-widest">
          PROJECT IDENTITY
        </h2>
        <p className="mt-1 font-sans text-sm text-text-muted">
          Tell the AI what you want to build. The more specific, the better the plan.
        </p>
      </div>
          ...
          <CardContent className="space-y-6 pt-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  variant={workspaceMode === "single" ? "primary" : "secondary"}
                  size="sm"
                ...
                <Button
                  variant={workspaceMode === "connection" ? "primary" : "secondary"}
                  size="sm"
                  onClick={() => onWorkspaceModeChange("connection")}
                >
                  Connection
                </Button>
              </div>
                ...
                <Field errorText={errors.workingDirectory}>
                  <div className="flex gap-2">
                    ...
                    <Button
                      variant="secondary"
                      size="md"
                      data-testid="describe-browse-directory"
                ...
                <Field errorText={errors.workingDirectory}>
                  {connections.length > 0 ? (
                    ...
                    <Card variant="ghost" className="border-border/60">
                      <CardContent className="p-3 font-sans text-sm text-text-dim">
...
Be specific about requirements, constraints, and goals."
            ...
            <CardContent className="space-y-3">
              {allAgents.map((agent) => {
                ...
                return (
                  <Button
                    ...
                    onClick={() => onPlanAgentChange(agent.name)}
                  >
              ...
              {capabilities?.supportsModel ? (
                <Field label="Model">
                  <Select
                    value={planModel ?? ""}
                    onValueChange={(val) => onPlanModelChange(val || null)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              {capabilities?.supportsEffort ? (
                <Field label="Effort">
                  <Select
                    value={planEffort ?? ""}
                    onValueChange={(val) => onPlanEffortChange(val || null)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
      ...
      <div className="mt-8 flex items-center justify-between border-border border-t pt-6">
        <Button variant="ghost" size="sm" data-testid="describe-cancel-button" onClick={onCancel}>
          Cancel process
        </Button>
        <Button
          variant="primary"
          size="md"
          type="submit"
          data-testid="describe-next-button"
          disabled={submitting}
        >
