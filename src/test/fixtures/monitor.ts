  ...
  return mergeFixture<AskCompletePayload>(
    {
      ...
      agent: "codex",
      model: "gpt-5.4",
      message: createAskMessage({ id: "message-002", content: "Final answer" }),
    },
    overrides,
  );
}

export function createAskErrorPayload(overrides?: DeepPartial<AskErrorPayload>): AskErrorPayload {
  return mergeFixture<AskErrorPayload>(
    {
      projectId: "project-001",
      messageId: "message-001",
      error: "Ask failed",
      message: createAskMessage({
        id: "message-003",
        content: "Error: Ask failed",
        agent: null,
        model: null,
      }),
    },
...
export function createAskQuestionResult(
  overrides?: DeepPartial<AskQuestionResult>,
): AskQuestionResult {
  return mergeFixture<AskQuestionResult>(
    {
      messageId: "message-001",
