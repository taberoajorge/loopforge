...
const CardHeader = React.forwardRef<HTMLDivElement, CardHeaderProps>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("flex flex-col gap-1.5 border-border/60 border-b p-4", className)}
        {...props}
      />
    );
  },
...
const CardTitle = React.forwardRef<HTMLHeadingElement, CardTitleProps>(
  ({ className, ...props }, ref) => {
    return (
      <h3
        ref={ref}
        className={cn("font-sans font-semibold text-sm text-text tracking-tight", className)}
        {...props}
      />
    );
  },
...
const CardDescription = React.forwardRef<HTMLParagraphElement, CardDescriptionProps>(
  ({ className, ...props }, ref) => {
    return (
      <p
        ref={ref}
        className={cn("font-sans text-text-muted text-xs leading-relaxed", className)}
        {...props}
      />
    );
  },
...
const CardFooter = React.forwardRef<HTMLDivElement, CardFooterProps>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn("flex items-center gap-2 border-border/60 border-t p-4", className)}
        {...props}
      />
    );
  },
